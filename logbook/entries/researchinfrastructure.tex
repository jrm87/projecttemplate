...
Collaboration occurs via GitHub issues for task assignments, GitHub code reviews, and logbook entries that share research designs and results.
Quality is enforced via GitHub Actions CI/CD checks on every pull request.
Code can run locally, on Duke's DCC cluster, or on TACC's Lonestar6 cluster,
with \texttt{generic.make} auto-detecting the environment.
Containerized environments (Apptainer/Singularity) ensure reproducible package versions across machines.
An agentic workflow using Claude Code provides AI-assisted planning, code review, and session continuity.
